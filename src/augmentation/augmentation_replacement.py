...
if __name__ == "__main__":
    ...
    # 원본 데이터를 복사한 후, random_masking_replacement 함수를 적용한 뒤 증강
    random_masking_replacement_train = orig_train.copy()

    print(f"증강 전 데이터 수: {len(random_masking_replacement_train)}")
    ...
    print(f"증강 후 데이터 수: {len(random_masking_replacement_train)}")  # ?

    # 증강 데이터 저장
    save_augmented_dataset(pd.concat([orig_train, random_masking_replacement_train]), 'sts/datasets/klue-sts-v1.1_train_random_masking_replacement_augset2.json')